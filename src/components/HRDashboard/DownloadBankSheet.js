...
const DownloadBankSheet = () => {
  ...
  const downloadExcel = () => {
    ...
    XLSX.utils.sheet_add_aoa(ws, [[`Company: ${companyInfo.name}`]], {
      origin: "A1",
    });
    XLSX.utils.sheet_add_aoa(ws, [[`Payroll Month: ${selectedMonth}`]], {
      origin: "A2",
    });
    ...
    <div className="p-6 max-w-7xl mx-auto">
      ...
      {loading ? (
        ...
        <p className="text-gray-500">
          No records found for selected month or search.
        </p>
      ) : (
            ...
            <tbody>
              {filteredData.map((emp) => (
                <tr key={emp.employeeId} className="hover:bg-gray-50">
                  ...
                  <td className="p-3 border">{emp.employeeName}</td>
                  <td className="p-3 border">{emp.bankName}</td>
                  <td className="p-3 border">{emp.accountNumber}</td>
                  <td className="p-3 border">{emp.ifscCode}</td>
                  <td className="p-3 border font-semibold">₹{emp.netSalary}</td>
                </tr>
              ))}
            </tbody>
          </table>
