...
const LeaveCalendar = () => {
  ...
  useEffect(() => {
    const fetchLeaves = async () => {
      try {
        const response = await axios.get("http://localhost:5133/api/Leave"); // ✅ Fixed endpoint
        setLeaves(response.data); // backend returns [{id, employee, type, from, to}]
      } catch (error) {
...
="calendar-leave-badges">
        <span className

="calendar-badge">{employees.length}</span>
      </div>
    ) : null;
  }
};
...
={`leave-calendar-container theme-${theme}`}>
...
="table-title">📅 Leave Calendar</div>
          <div className
...
="employee-name">{leave.employee}</span>
                      <span className

={`status-pill ${leave.type.replace(" ", "-")}`}>
                        {leave.type}
                      </span>
                    </div>
                  </div>
...
="noData">✅ No leaves on this day</li>
            )}
          </ul>
        </div>
