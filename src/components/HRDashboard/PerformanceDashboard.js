...
const PerformanceDashboard = () => {
  ...
  const monthlySales = monthLabels.map((m) =>
    kpis
      .filter((k) => k.month === m)
      .reduce((sum, x) => sum + (x.totalSales || 0), 0),
  );

  const salesTarget = monthLabels.map((m) =>
    kpis
      .filter((k) => k.month === m)
      .reduce((sum, x) => sum + (x.salesTarget || 0), 0),
  );
  ...
  const monthlyInvoices = monthLabels.map((m) =>
    kpis
      .filter((k) => k.month === m)
      .reduce((sum, x) => sum + (x.invoicesProcessed || 0), 0),
  );
  const monthlyPayments = monthLabels.map((m) =>
    kpis
      .filter((k) => k.month === m)
      .reduce((sum, x) => sum + (x.paymentsCleared || 0), 0),
  );
  ...
  const monthlyLeads = monthLabels.map((m) =>
    kpis
      .filter((k) => k.month === m)
      .reduce((sum, x) => sum + (x.leadsGenerated || 0), 0),
  );

  const monthlyRevenue = monthLabels.map((m) =>
    kpis
      .filter((k) => k.month === m)
      .reduce((sum, x) => sum + (x.revenueGenerated || 0), 0),
  );
  ...
  return (
    <div className="perf-container">
      <h2 className="title">
        <FiBarChart2 className="title-icon" size={40} />
      ...
      <div className="tabs">
        <div
          className={`tab-item sales-tab ${activeTab === "sales" ? "active" : ""}`}
          onClick={() => setActiveTab("sales")}
        >
          <FaChartLine className="tab-icon" /> Sales KPI
        </div>

        <div
          className={`tab-item it-tab ${activeTab === "it" ? "active" : ""}`}
          onClick={() => setActiveTab("it")}
        >
          <FaLaptopCode className="tab-icon" /> IT KPI
        </div>

        <div
          className={`tab-item hr-tab ${activeTab === "hr" ? "active" : ""}`}
          onClick={() => setActiveTab("hr")}
        >
          <FaUserTie className="tab-icon" /> HR KPI
        </div>

        <div
          className={`tab-item finance-tab ${activeTab === "finance" ? "active" : ""}`}
          onClick={() => setActiveTab("finance")}
        >
          <FaMoneyBillWave className="tab-icon" /> Finance KPI
        </div>

        <div
          className={`tab-item marketing-tab ${activeTab === "marketing" ? "active" : ""}`}
          onClick={() => setActiveTab("marketing")}
        >
          <FaChartLine className="tab-icon" /> Marketing KPI
        </div>
                ...
                {kpis.map((k) => (
                  <tr key={k.id}>
                    <td>
                      {k.employeeName ||
                        (k.employee
                          ? `${k.employee.firstName} ${k.employee.lastName}`
                          : "N/A")}
                    </td>
                    <td>
                      {k.month
                        ? new Date(k.month).toLocaleDateString("en-GB")
                        : "—"}
                    </td>
                    ...
                    <td className="score">{k.kpiScore ?? "—"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
