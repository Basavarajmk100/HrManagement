...
const PayrollDashboard = () => {
  ...
  const fetchPayroll = async () => {
    try {
      ...
      const excelOnlyRows = payrolls
        .filter(
          (p) => !employees.some((e) => String(e.empId) === String(p.empId)),
        )
        .map((p) => ({
        ...
        const mappedData = rawData
          .map((row, index) => {
            ...
            const payrollMonth = String(
              row["PayrollMonth"] || selectedMonth || "",
            ).trim();
  ...
  return (
    <div className={styles.payrollDashboardContainer}>
      {/* HEADER */}
      <h2 className="payroll-header-title">
        <RiMoneyRupeeCircleFill className="payroll-icon" />
        Payroll
      </h2>
      ...
      >
        ...
        >
          ...
          {[
            ...new Set(
              payrollList.map((p) => p.payrollMonth).filter((month) => month), // remove null/undefined
            ),
          ]
            .sort((a, b) => {
              const monthOrder = [
                ...
                .map((emp, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td>
                        {emp.salaryCalendarDays !== ""
                          ? emp.salaryCalendarDays
                          : "-"}
                      </td>
                      <td>{emp.weeklyOff !== "" ? emp.weeklyOff : "-"}</td>
                      <td>
                        {emp.generalHolidays !== "" ? emp.generalHolidays : "-"}
                      </td>
                      <td>{emp.uan || "-"}</td>
